...
export function clientError(response: Response, message?: string) {
  return jsonResponse(response, 400, AppErrorType.UNAUTHORIZED, message ? message : '');
}

export function unauthorized(response: Response, message?: string) {
  return jsonResponse(response, 401, AppErrorType.UNAUTHORIZED, message ? message : '');
}

export function forbidden(response: Response, message?: string) {
  return jsonResponse(response, 403, AppErrorType.FORBIDDEN, message ? message : '');
}

export function notFound(response: Response, message?: string) {
  return jsonResponse(response, 404, AppErrorType.RESOUCE_NOT_FOUND, message ? message : '');
}

export function conflict(response: Response, message?: string) {
  return jsonResponse(response, 409, AppErrorType.CONFLICT_ERROR, message ? message : '');
}

export function fail(response: Response, error?: Error) {
  console.log(error);
  if (error instanceof AppError) {
    return jsonResponse(response, error.status, error.type, error?.userMessage)
  } else {
    return jsonResponse(response, 500, AppErrorType.SYSTEM_ERROR, error?.message)
  }
}
